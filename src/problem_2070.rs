...
impl Solution {
    pub fn maximum_beauty(items: Vec<Vec<i32>>, queries: Vec<i32>) -> Vec<i32> {
        ...
        queries
            .iter()
            .map(|query| *beauty_map.get(query).unwrap())
            .collect()
    }
    ...
    fn sort_array(items: Vec<i32>) -> Vec<i32> {
        ...
        Self::merge_arrays(
            &Self::sort_array(items[0..size].to_vec()),
            &Self::sort_array(items[size..].to_vec()),
        )
    }

    fn merge_arrays(array_1: &[i32], array_2: &[i32]) -> Vec<i32> {
        let mut i = 0;
        let mut j = 0;
        let mut ret = vec![];

        while i < array_1.len() && j < array_2.len() {
            let elem_1 = array_1[i];
            let elem_2 = array_2[j];
            if elem_1 < elem_2 {
                ret.push(elem_1);
                i += 1;
            } else {
...
mod tests {
    ...
    #[test]
    fn test_1() {
        let ok = Solution::maximum_beauty(
